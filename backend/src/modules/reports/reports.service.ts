...
export class ReportsService {
  ...
  async sales(from?: string, to?: string) {
    ...
    return { count: rows.length, total: Number(total.toFixed(2)), items: rows };
  }

  async inventory() {
    const rows = await this.productRepo.find({ where: { pharmacyId: this.pid }, order: { name: 'ASC' } });
    const totalStockQty = rows.reduce((s, r) => s + r.stockQuantity, 0);
    const totalStockValue = rows.reduce((s, r) => s + r.stockQuantity * Number(r.costPrice), 0);
    return { totalStockQty, totalStockValue: Number(totalStockValue.toFixed(2)), items: rows };
  }

  async profitLoss(from?: string, to?: string) {
    const pid = this.pid;
    const salesQb = this.saleRepo.createQueryBuilder('s').where('s.status = :st', { st: 'completed' }).andWhere('s.pharmacy_id = :pid', { pid });
    const expenseQb = this.expenseRepo.createQueryBuilder('e').where('e.pharmacy_id = :pid', { pid });
    const itemQb = this.saleItemRepo
      .createQueryBuilder('si')
      .innerJoin(Sale, 's', 's.id = si.sale_id')
      .where('s.status = :st', { st: 'completed' })
      .andWhere('s.pharmacy_id = :pid', { pid });
    if (from) {
      salesQb.andWhere('DATE(s.sale_date) >= :from', { from });
      expenseQb.andWhere('e.expense_date >= :from', { from });
      itemQb.andWhere('DATE(s.sale_date) >= :from', { from });
    }
    if (to) {
      salesQb.andWhere('DATE(s.sale_date) <= :to', { to });
      expenseQb.andWhere('e.expense_date <= :to', { to });
      itemQb.andWhere('DATE(s.sale_date) <= :to', { to });
    }
    const [sales, expenses, items] = await Promise.all([salesQb.getMany(), expenseQb.getMany(), itemQb.getMany()]);
    const revenue = sales.reduce((s, r) => s + Number(r.totalAmount), 0);
    const cost = items.reduce((s, i) => s + i.quantity * Number(i.costPrice), 0);
  ...
  async expiry(days = 30) {
    const rows = await this.productRepo
      .createQueryBuilder('p')
      .where('p.pharmacy_id = :pid', { pid: this.pid })
      .andWhere('p.expiry_date IS NOT NULL')
      .andWhere(`p.expiry_date <= CURRENT_DATE + INTERVAL '${days} days'`)
      .orderBy('p.expiry_date', 'ASC')
      .getMany();
    return { days, count: rows.length, items: rows };
  }
